...
function Player(name, cardTable = null) {
  ...
  this.plays = () => {
    ...
    if (!flag && !this.isBomb()) {
      if (this.name === 'player1') {
        this.cardTable.showTips('你没有牌可以大过大家~~') // 你没有牌可以大过大家
      } else {
        this.passPlays()
        return
      }
  ...
  this.playerPlays = function() {
    ...
    if (flag) {
      ...
      // 删除手中的牌
      for (let i = 0; i < cards.length; i++) {
        const index = this.box.indexOf(cards[i])
        if (index !== -1) {
  ...
  this.computerPlays = function () {
    ...
    if (this.root) {
      ...
      if(this.waitBox.length) {
        ...
        // 删除自身
        for (let i = 0; i < this.waitBox.length; i++) {
          const index = this.box.indexOf(this.waitBox[i])
          if (index !== -1) {
            this.box.splice(index, 1)
          }
        }
        ...
        this.box.splice(this.box.length - count, count + 1) // 删除自身
      }
      flag = true // 成功出牌
    } else {
      outArr = []
      ...
      if (this.waitBox.length) {
        ...
        for (let i = this.box.length - 1; i >= this.cardTable.typeCount; i--) {
          ...
          if (this.box[i].size > rootCard.size) {
            ...
            if (this.getCountByType(this.cardTable.TYPE) !== count) {
              // 如果不一样，就是存在对子以上、
              i = i - (count - 1)
              continue
            }
    ...
    if (!flag) {
      outArr = []
      let bombFlag = false
      if (this.isBomb() && this.cardTable.TYPE !== 'four') {
  ...
  this.cardLineRule = function (index) {
    // 先添加该元素
    this.waitBox.push(this.box[index])
    let count = 1
    // 定义两个指针
    let q = index - 1
    let p = index
    for (; q >= 0;) {
      ...
      const dif = Math.abs(this.box[q].size - this.box[p].size)
      if (dif === 0) {
        q--
        continue
      }
      if (dif === 1) { // 差值等于 1
        ...
        if (count >= this.cardTable.LAST_PLAY.length) {
          break
        }
        p = q
        q--
    ...
    return count >= this.cardTable.LAST_PLAY.length
  }
